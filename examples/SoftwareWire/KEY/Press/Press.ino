// ПРИМЕР РЕАГИРОВАНИЯ НА НАЖАТИЕ ОДНОЙ КНОПОКИ:                                       //
...
//  Проверяем наличие модулей Metro на шине I2C:                                       //
    ...
    while(Metro[0].model!=MOD_KEY){;}                                                  // Если ближайший к Arduino модуль Metro не является кнопкой, то так же входим в бесконечный цикл while.
                                                                                       ...
                                                                                       //
void loop(){                                                                           //
//  Включаем светодиод на плате Arduino пока нажата кнопка:                            //
    digitalWrite ( LED_BUILTIN,  Metro[0].read(KEY_PRESSED)  );                        // Функция read с параметром KEY_PRESSED возвращает true если кнопка нажата и false если отпущена.
}                                                                                      //