// ПРИМЕР ВКЛЮЧЕНИЯ И ОТКЛЮЧЕНИЯ ВСЕХ ПИКСЕЛЕЙ ОДНОГО СВЕТОДИОДНОГО ИНДИКАТОРА:        //
...
//  Выключаем дисплей:                                                                 //
    Metro[0].off();                                                                    // Если функция off() вызвана без параметров, то весь дисплей будет очищен.
    delay(2000);                                                                       // Ждём 2 секунды.
}                                                                                      //