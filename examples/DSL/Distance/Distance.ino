// Пример определения уровня близости препятствия для одного или нескольких датчиков:  //
...
//  Готовимся выводить информацию в монитор последовательного порта:                   //
    ...
    Serial.println( "Текст выводится при наличии препятствий перед датчиками." );      // Выводим текст.
}                                                                                      //
                                                                                       //
void loop(){                                                                           //
    uint8_t j=0;                                                                       // Определяем переменную для хранения порядкового номера датчика освещённости.
    uint8_t L=0;                                                                       // Определяем переменную для хранения уровня близости препятствия.
...
//      Если очередной модуль i является датчиком освещённости:                        //
        if(Metro[i].model==MOD_DSL){                                                   // Тип модуля определяется по значению model.
            ...
            L = Metro[i].read( DSL_DISTANCE );                                         // Читаем уровень близости препятствия к датчику.
            if(L){                                                                     // Если перед датчиком есть препятствие, то ...
                Serial.print  ("Датчик N ");                                           // Выводим текст.
                Serial.print  (j);                                                     // Выводим порядковый номер датчика освещённости.
                Serial.print  (": уровень близости препятствия = ");                   // Выводим текст.
                Serial.println( L );                                                   // Выводим уровень близости препятствия.
    ...
    delay(1000);                                                                       // Ставим задержку на 1 секунду. Это предотвратит быстрое заполнение монитора строками "Датчик N ... : уровень близости препятствия = ..."
}                                                                                      //
