// Определение коэффициента пульсаций источника света для одного/нескольких датчиков:  //
...
//      Если очередной модуль i является датчиком освещённости:                        //
        if(Metro[i].model==MOD_DSL){                                                   // Тип модуля определяется по значению model.
            ...
            Serial.print  (": коэффициент пульсаций источника света = ");              // Выводим текст.
            Serial.print  ( Metro[i].read( DSL_COEFFICIENT ) );                        // Выводим коэффициент пульсаций источника света.
    ...
    Serial.println("========================================");                        // Выводим текст с переводом строки.
    delay(1000);                                                                       // Ставим задержку на 1 секунду. Это предотвратит быстрое заполнение монитора строками "Датчик N ... коэффициент пульсаций источника света = ... %.".
}                                                                                      //
