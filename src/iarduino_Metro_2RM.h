		...
		bool	action			(int i0=NOT2, int i1=NOT2	){											//	
								...
								//	Действие № 0 - прочитать состояние реле:							//
									case 0:																//
										...
										do{	result	 =	objI2C.readBytes( i1, REG_DATA+2, data, 1);		//	Читаем 1 байт из модуля с адресом i1, начиная с регистра REG_DATA+2 в массив data.
								...
								//	Действие № 1 - отключить реле:										//
									case 1:																//
										...
										do{	result = objI2C.writeBytes( i1, REG_DATA+3, data, 1);		//	Записываем 1 элемент массива data в модуль с адресом i1, начиная с регистра REG_DATA+3.
								...
								//	Действие № 3 - установить время сторожевого таймера:				//
									case 3:																//
										...
										do{	result = objI2C.writeBytes( i1, REG_DATA+0x20, data, 1);	//	Записываем 1 элемент массива data в модуль с адресом i1, начиная с регистра REG_DATA+0x20.
											sumtry	--;	if(!result){delay(1);}							//	Уменьшаем количество попыток записи и устанавливаем задержку при неудаче.
										}	while		(!result && sumtry>0);							//	Повторяем запись если она завершилась неудачей, но не более sumtry попыток.
									break;																//
								}																		//
